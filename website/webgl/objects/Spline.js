...
class Spline {
	...
	*/
	...
	initTimeline() {
		...
		this.timeline.to(this.tweenTime, 10, {
			time: 1,
			onComplete: () => {
	...
	reverseTimeline() {
		...
		this.timeline.fromTo(this.zoomParams, 1.5, { strength: 0.5 }, {
			strength: 0.0025,
			ease: Circ.easeInOut,
			onComplete: () => {
				this.disableSpline();
				TweenMax.to(this.controlsContainer.rotation, 0.6, {
					y: 0,
					onComplete: () => {
